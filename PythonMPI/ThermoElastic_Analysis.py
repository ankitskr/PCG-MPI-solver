...
def readModelData(GlobData):
    ...
    GlobData.update(RefMeshPart['GlobData']) <--- check if this works
...
def calcMPFint(MP_Xn, ComputeReference, RefMeshPart):
    ...
    for j in range(N_Type):        
        ...
        elif ComputeReference == 'Coupling': 
            
            Kc                  = RefTypeGroup['ElemThermCplMat'] #Thermal Coupling Matrix
            ElemList_SignVector = RefTypeGroup['ElemList_SignVector']
            ElemList_Cc         = RefTypeGroup['ElemList_Cc']
            
            ElemList_Tn         = MP_Xn[ElemList_LocNodeIdVector]
            ElemList_RefVec     = np.dot(Kc, ElemList_Cc*ElemList_Tn)
...
def calcMPTemp(MP_Xn, ComputeReference, RefMeshPart):

        
    MP_SubDomainData            = RefMeshPart['SubDomainData']
    MP_NbrMPIdVector            = RefMeshPart['NbrMPIdVector']
    MP_OvrlpLocalNodeIdVecList  = RefMeshPart['OvrlpLocalNodeIdVecList']
    Flat_ElemLocNodeId          = RefMeshPart['Flat_ElemLocNodeId']
    NCountNodeId                = RefMeshPart['NCountNodeId']
    ...
    #Calculating Local RefVec Vector for Octree cells
    MP_LocRefVec = np.zeros(MP_NNode, dtype=float)   
    if RefVecCalcMode == 'outbin':
        Flat_ElemRefVec = np.zeros(NCountNodeId, dtype=float)
        I=0     
    ...
    for j in range(N_Type):        
        RefTypeGroup                    = MP_TypeGroupList[j]    
        ElemList_LocNodeIdVector        = RefTypeGroup['ElemList_LocNodeIdVector']
        ElemList_LocNodeIdVector_Flat   = RefTypeGroup['ElemList_LocNodeIdVector_Flat']
        ElemList_LocDofVector           = RefTypeGroup['ElemList_LocDofVector']
        
        if ComputeReference == 'Thermal':
            
            Kt                 = RefTypeGroup['ElemThermCdcMat'] #Thermal Conductivity Matrix
            ElemList_Ct        = RefTypeGroup['ElemList_Ct']
            
            ElemList_Tn         = MP_Xn[ElemList_LocNodeIdVector]
            ElemList_RefVec     = np.dot(Kt, ElemList_Ct*ElemList_Tn)
            ...
            ElemList_dUn         = MP_Xn[ElemList_LocNodeIdVector]
            ElemList_RefVec     = np.dot(Kc.T, ElemList_Cc*ElemList_dUn)
...
def calcMPTempVec(MP_Tn_2, MP_Qext_1, MP_QintVec_1, MP_QuVec_1, RefMeshPart):
    
    dt                      = RefMeshPart['GlobData']['dt']
    MP_LocFixedTNodeId      = RefMeshPart['LocFixedTempNodeId']
    MP_InvDiagCt            = RefMeshPart['InvDiagCt']
    MP_Td                   = RefMeshPart['Td']
    
    MP_Tn = MP_Tn_2 + 2*dt*MP_InvDiagCt*(MP_Qext_1 - MP_QintVec_1 - MP_QuVec_1/(2*dt))
    MP_Tn[MP_LocFixedTNodeId] = MP_Td[MP_LocFixedTNodeId]
    
    RefMeshPart['MP_Tn'] = MP_Tn
...
def updatePlotData(RefMeshPart):
...
if __name__ == "__main__":
    ...
    configGlobalSettings(GlobData)
    
    MP_NDOF                 = RefMeshPart['NDOF']
    MP_NNode                = RefMeshPart['NNode']
    MP_RefLoadVector        = RefMeshPart['RefLoadVector']
    MP_RefHeatFlowVector    = RefMeshPart['RefHeatFlowVector']
    RefMaxTimeStepCount     = GlobData['RefMaxTimeStepCount']
    TimeStepDeltaF          = GlobData['TimeStepDeltaF']
    TimeStepDeltaQ          = GlobData['TimeStepDeltaQ']
    ...
    MP_Tn_1 = (1e-200)*np.random.rand(MP_NNode)
    MP_Tn   = (1e-200)*np.random.rand(MP_NNode)
    ...
    for TimeStepCount in range(1, RefMaxTimeStepCount):
        ...
        DeltaLambdaF_1 = TimeStepDeltaF[TimeStepCount-1]
        MP_Fext_1 = MP_RefLoadVector*DeltaLambdaF_1 #External Force Vector
        MP_FintVec_1 = calcMPFint(MP_Un_1, 'Strain', RefMeshPart) #Internal Force due to strain
        MP_FthVec_1 = calcMPFint(MP_Tn_1, 'Coupling', RefMeshPart) #Internal Force due to thermal coupling
        MP_Un = calcMPDispVec(MP_Un_2, MP_Un_1, MP_FintVec_1, MP_FthVec_1, MP_Fext_1, RefMeshPart)
        
        DeltaLambdaQ_1 = TimeStepDeltaQ[TimeStepCount-1]
        MP_Qext_1 = MP_RefHeatFlowVector*DeltaLambdaQ_1 #External HeatFlow Vector
        MP_QintVec_1 = calcMPQint(MP_Tn_1, 'Thermal', RefMeshPart) #Internal HeatFlow due to temperature
        MP_QuVec_1 = calcMPQint(MP_Un - MP_Un_2, 'Coupling', RefMeshPart) #Internal HeatFlow due to strain coupling
        MP_Tn = calcMPTempVec(MP_Tn_2, MP_Qext_1, MP_QintVec_1, MP_QuVec_1, RefMeshPart)
    ...
    Glob['MP_TimeRecData']['dT_Total_Verify'] = t0_End - t0_Start 
    Glob['MP_TimeRecData']['t0_Start'] = t0_Start
    Glob['MP_TimeRecData']['t0_End'] = t0_End
    Glob['MP_TimeRecData']['MP_NDOF'] = MP_NDOF
