...
def config_GlobData(MatDataPath, MeshData_Glob_FileName):
...
if __name__ == "__main__":
    ...
    NodeGlbFlat_FileName         = MatDataPath + 'NodeGlbFlat.bin'
    NodeGlbOffset_FileName       = MatDataPath + 'NodeGlbOffset.bin'
    Type_FileName               = MatDataPath + 'Type.bin'
    Level_FileName              = MatDataPath + 'Level.bin'
    MetisSettings_FileName      = MatDataPath + 'MetisSettings.mat'
    MeshData_Glob_FileName      = MatDataPath + 'MeshData_Glob.zpkl'
    
    t1 = time()
    
    if Rank==0: config_GlobData(MatDataPath, MeshData_Glob_FileName)
    Comm.barrier()
    ...
    """
    #Partitioning based on Level (for Non-local damage)
    RefLc = 3.2*0.14e-3
    RefLevel=np.array(Level);
    RefLevel[Level>RefLc]=RefLc
    WtGlb = 1.0/(10e-6+RefLevel)
    #WtGlb /=np.max(WtGlb)
    """
    
    """
    #Partitioning based on Type
    KeDataFile = MatDataPath + 'Ke.mat'
    Ke = scipy.io.loadmat(KeDataFile)['Data'][0]
    N_UnqTypes = len(Ke)
    NDofList = np.array([len(Ke[i]) for i in range(N_UnqTypes)],dtype=float)
    MaxNDof = np.max(NDofList)
    NDofTypeList = NDofList[Type]
    WtGlb = (3*MaxNDof+NDofTypeList)/(4*MaxNDof)
    """
    ...
    OutputFileName = MatDataPath + 'MeshPart_' + str(RefMeshPrts)
