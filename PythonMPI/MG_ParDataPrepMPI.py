...
def initGlobDataPrep():
    ...
    MaxMGLevel      = scipy.io.loadmat(ModelDataPath+'MaxMGLevel.mat')['MaxMGLevel'][0][0]
...
def extract_Elepart(GlobData):
    ...
    for m in range(MaxMGLevel):
        ...
        MPGData_m['GlobData'].update(MeshData_Glob)
        GlobNElem = MPGData_m['GlobData']['GlobNElem']
        ...
        if LocalRank == 0:
            MeshPartFile = MatDataPath + 'MeshPart_' + str(N_TotalMshPrt) + '_L' + str(m+1) '.npy'
        ...
        MPGData_m['RefElemIdVecList'] = RefElemIdVecList
        MPGData_m['MPIdList']         = MPIdList
...
def extract_PlotSettings(MGDataList):
    
    MaxMGLevel = len(MGDataList)
    for m in range(MaxMGLevel):
        
        MPGData_m = MGDataList[m]
        
        #Reading Global data file
        GlobData    = MPGData_m['GlobData']
        MatDataPath = GlobData['MatDataPath']
...
def extract_ElemMeshData(MGDataList):
    ...
    for m in range(MaxMGLevel):
        ...
        GlobData            = MPGData_m['GlobData']
        RefElemIdVecList    = MPGData_m['RefElemIdVecList']
        
        MPGData_m['MeshPartList'] = []
        MPIdList = MPGData_m['MPIdList']
        MPGSize = len(MPIdList)
        MPGData_m['MPGSize'] = MPGSize
        ...
        for i in range(MPGSize):
            ...
            MeshPart['Id']          = MP_Id
            MeshPart['IntfcElem']   = []
            MPGData_m['MeshPartList'].append(MeshPart)
        ...
        for j in range(N_Data):
            ...
            for i in range(MPGSize):
                ...
                MeshPart =  MPGData_m['MeshPartList'][i]
            ...
            if DataName in ['Type', 'Level', 'sctrs']:
                MPGData_m['Glob'+DataName] = RefElemData
...
def config_NonlocalNeighbours(MPGData):
    ...
    N_Mat                       = len(MatProp)
    Ko                          = 3.2
    TODO: Compare with DynElast_ParDataPrepMPI.py
...
if __name__ == "__main__":
    ...
    MGDataList = extract_Elepart(GlobData)
    extract_PlotSettings(MGDataList)
    
    if Rank==0: print(Rank, 'A')
    
    extract_ElemMeshData(MGDataList)
    Comm.barrier()
