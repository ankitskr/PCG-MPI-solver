...
def initGlobDataPrep():
    ...
    ExportNonLocalStress =  bool(int(sys.argv[3]))
...
def extract_ElemMeshData(MPGData):
    ...
    DataNameList =    [ 'NodeGlbOffset',    'DofGlbOffset',       'SignOffset',       'Type',       'Level',         'Ckd',         'Ckv',          'Cm',          'Ce',    'PolyMat',        'sctrs',        'StrsGlb',       'StrsSign']
    BinDataTypeList = [       np.int64,          np.int64 ,           np.int64,     np.int32,    np.float64,    np.float64,    np.float64,    np.float64,    np.float64,     np.int32,     np.float64,          np.int8,          np.int8]
    PyDataTypeList =  [            int,               int ,                int,          int,         float,         float,         float,         float,         float,          int,          float,              int,             bool]
    DataShapeList =   [  (GlobNElem,2),     (GlobNElem,2) ,      (GlobNElem,2), (GlobNElem,),  (GlobNElem,),  (GlobNElem,),  (GlobNElem,),  (GlobNElem,),  (GlobNElem,), (GlobNElem,),  (GlobNElem,3),   (GlobNElem, 6),   (GlobNElem, 6)]
...
def config_ElemVectors(MPGData):
    ...
    SignFlat    = loadBinDataInSharedMem(MatDataPath + 'SignFlat.bin', np.int8, bool, (GlobNDofGlbFlat,), Comm, LoadingRank=LRList[0], Wait=WList[0])
    NodeGlbFlat  = loadBinDataInSharedMem(MatDataPath + 'NodeGlbFlat.bin', np.int32, int, (GlobNNodeGlbFlat,), Comm, LoadingRank=LRList[1], Wait=WList[1])
    DofGlbFlat  = loadBinDataInSharedMem(MatDataPath + 'DofGlbFlat.bin', np.int32, int, (GlobNDofGlbFlat,), Comm, LoadingRank=LRList[2], Wait=WList[2])
...
def extract_NodalVectors(MPGData):
    ...
    DiagM           = loadBinDataInSharedMem(MatDataPath + 'DiagM.bin', np.float64, float, (GlobNDof,), Comm, LoadingRank=LRList[0], Wait=WList[0])
    F               = loadBinDataInSharedMem(MatDataPath + 'F.bin', np.float64, float, (GlobNDof,), Comm, LoadingRank=LRList[1], Wait=WList[1])
    DofEff          = loadBinDataInSharedMem(MatDataPath + 'DofEff.bin', np.int32, int, (GlobNDofEff,), Comm, LoadingRank=LRList[2], Wait=WList[2])
    FixedDof        = loadBinDataInSharedMem(MatDataPath + 'FixedDof.bin', np.int32, int, (GlobNFixedDof,), Comm, LoadingRank=LRList[3], Wait=WList[3])
    Ud              = loadBinDataInSharedMem(MatDataPath + 'Ud.bin', np.float64, float, (GlobNDof,), Comm, LoadingRank=LRList[4], Wait=WList[4])
    Vd              = loadBinDataInSharedMem(MatDataPath + 'Vd.bin', np.float64, float, (GlobNDof,), Comm, LoadingRank=LRList[5], Wait=WList[5])
    NodeCoordVec    = loadBinDataInSharedMem(MatDataPath + 'NodeCoordVec.bin', np.float64, float, (GlobNDof,), Comm, LoadingRank=LRList[6], Wait=WList[6])
...
def config_TypeGroupList(MPGData):
    ...
    for i in range(N_MeshPart):
        ...
        for j in range(N_Type):
            ...
            RefElemList_Ckd             = MP_Ckd[I]
            RefElemList_Ckv             = MP_Ckv[I]
            ...
            MP_TypeGroup['ElemList_Ckd']                 = RefElemList_Ckd
            MP_TypeGroup['ElemList_Ckv']                 = RefElemList_Ckv
...
def config_ElemLib(MPGData):
    ...
    KdDataFile          = MatDataPath + 'Kd.mat'
    KvDataFile          = MatDataPath + 'Kv.mat'
    MeDataFile          = MatDataPath + 'Me.mat'
    #SeDataFile          = MatDataPath + 'Se.mat'
    Kd                  = scipy.io.loadmat(KdDataFile)['Data'][0]
    Kv                  = scipy.io.loadmat(KvDataFile)['Data'][0]
    ...
    for i in range(N_MeshPart):
        ...
        for j in range(N_Type):
            ...
            if RefElemTypeId in [-2, -1]: #Interface Elements
                ...
                RefTypeGroup['ElemStiffMat_v']        = np.array(Kv[j0], dtype=float)
                RefTypeGroup['ElemStiffMat_d']        = np.array(Kd[j0], dtype=float)
                ...
                RefTypeGroup['NNodes']              = int(len(Kd[j0])/3)
...
def config_NonlocalNeighbours_Old(MPGData):
    ...
    for i in range(N_MeshPart):
        ...
        for j in range(MP_NElem):
            ...
            #To achieve this, save Elem_WeightVec and its location in a separate variable
...
def config_NonlocalNeighbours(MPGData):
    ...
    if not (DmgProp['Type'] == 'NonLocal' or GlobData['ExportNonLocalStress']):    return
    ...
    if DmgProp['Type'] == 'NonLocal':
        RefLc                       = Ko*np.max([MatProp[i]['SofteningParam']['Lc'] for i in range(N_Mat)])
    elif GlobData['ExportNonLocalStress']:
        #RefLc                       = Ko*np.max(MPGData['GlobLevel'])*np.max([MatProp[i]['NonLocStressParam']['LcFac'] for i in range(N_Mat)])
    ...
    for i in range(N_MeshPart):
        ...
        for j in range(MP_NElem):
            ...
            if DmgProp['Type'] == 'NonLocal':
                ...
                #Lc                  = MP_Level[j]*MatProp[MP_PolyMat[j]]['NonLocStressParam']['LcFac']
...
def exportMP(MPGData):
    ...
    RefKeyList = ['Id', 'SubDomainData', 'NDOF', 'NNode', 'DofVector', 'NodeIdVector', 'InvDiagM', 'NodeWeightVector', \
                  ...
                  'DmgProp', 'MatProp', 'NodeCoordVec']
                  
    DmgProp = MPGData['DmgProp']
    if DmgProp['Type'] == 'NonLocal' or GlobData['ExportNonLocalStress']:
...
if __name__ == "__main__":
    ...
    MPGData['PotentialNbrDataFlag'] = False
    
    RefElemIdVecList, MPIdList = extract_Elepart(MPGData, MatDataPath, N_TotalMshPrt, N_MPGs, Rank)
    extract_PlotSettings(MPGData, MatDataPath)
    MPGData['MPIdList'] = MPIdList
    
    Comm.barrier()
    
    if N_Workers == 1:
        extract_ElemMeshData(MPGData, MatDataPath, RefElemIdVecList, Rank)
        Comm.barrier()
    ...
    if Rank==0: 
        t1 = time()
        print(Rank, 'B0')
    config_ElemVectors(MPGData, Rank)
    Comm.barrier()
    if Rank==0: 
        print(time()-t1)
        print('')
    ...
    if Rank==0: 
        t1 = time()
        print(Rank, 'B1')
    extract_NodalVectors(MPGData, MatDataPath, Rank)
    Comm.barrier()
    if Rank==0: 
        print(time()-t1)
        print('')
    ...
    if Rank==0: 
        t1 = time()
        print(Rank, 'D')
    config_ElemMaterial(MPGData, MatDataPath)
    config_ElemLib(MPGData, MatDataPath)
    Comm.barrier()
    if Rank==0: 
        print(time()-t1)
        print('')
