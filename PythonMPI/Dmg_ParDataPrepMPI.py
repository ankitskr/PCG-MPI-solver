...
def extract_Elepart(MPGData, MatDataPath, N_TotalMshPrt, N_MPGs, Rank):
...
def extract_PlotSettings(MPGData, MatDataPath):
    ...
    MeshData_Glob['qpoint'] = qpoint.reshape((Nqpts,3),order='F')
    
    MeshData_Glob['RefPlotDofVec'] = np.fromfile(MatDataPath+'RefPlotDofVec.bin', dtype=np.int64).astype(int)
    


    
def extract_ElemMeshData(MPGData, MatDataPath, RefElemIdVecList, Rank):
...
def config_ElemVectors(MPGData, Rank):
    
    #print('Computing element vectors..<<>>..')
    #A small sleep to avoid hanging
    sleep(Rank*1e-3)
    GlobNNodeGlbFlat = MPGData['Glob']['GlobNNodeGlbFlat']
    GlobNDofGlbFlat = MPGData['Glob']['GlobNDofGlbFlat']
...
def extract_NodalVectors(MPGData, MatDataPath, Rank):
        
    #print('Computing Nodal vectors..')
    GlobNDof    = MPGData['Glob']['GlobNDof']
    GlobNFixedDof    = MPGData['Glob']['GlobNFixedDof']
    GlobNDofEff = MPGData['Glob']['GlobNDofEff']
...
def config_ElemMaterial(MPGData, MatDataPath):
...
def config_ElemLib(MPGData, PyDataPath):
...
def config_NonlocalNeighbours(MPGData):
    ...
    if not (DmgProp['Type'] == 'NonLocal' or MPGData['ExportNonLocalStress']):    return
    ...
    if DmgProp['Type'] == 'NonLocal':
        RefLc                       = Ko*np.max([MatProp[i]['SofteningParam']['Lc'] for i in range(N_Mat)])
    elif MPGData['ExportNonLocalStress']:
...
def exportMP(MPGData, PyDataPath_Part):
    ...
    TotalMeshPartCount = MPGData['TotalMeshPartCount']
    Data_FileName = PyDataPath_Part + str(TotalMeshPartCount)
    if Rank == 0:
...
if __name__ == "__main__":
    ...
    MPGData['ExportNonLocalStress'] = ExportNonLocalStress
