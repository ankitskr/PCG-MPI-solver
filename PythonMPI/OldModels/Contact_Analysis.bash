export ModelName=Contact_cuboid_Hex_6
NCPUS=(       48   48   48   48   48   48   48   96  192  384   786   1536)
N_MshPrts=(    1    2    3    6   12   24   48   96  192  384   768   1536)
TotMemList=( 190  190  190  190  190  190  190  360  720 1440  2880   5760)
...
for P0 in 0 1 2 3 4 5 6 7 8; do 
    ...
    qsub -N $JobName -P $ProjName -q $Queue -l ncpus=$NCPU,walltime=$WallTime,mem=$TotMem,storage=gdata/$ProjName -M $Email -m ae -o $LogPath -j oe -v ModelName,WorkDir,ScratchPath,N_MshPrt,R0,SpeedTestFlag Contact_Analysis.pbs;
