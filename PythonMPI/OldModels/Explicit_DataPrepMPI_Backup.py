...
def readLoadConfig(MatPath):
    ...
    RefDOFIdList =  TimeData['RefPlotDofId'].T[0] - 1
...
def configMP_GlobVectors(MeshPartList, F, M, Fixed):
    ...
    for i in range(N_MeshPart):
        ...
        for j in range(i, N_MeshPart):
            ...
            if not i == j:
                ...
                OvrlpDOFIdVector = np.intersect1d(MP_DOFIdVector_i,MP_DOFIdVector_j)
...
def main(p, ScratchPath):
    ...
    Fixed, F, GlobData = readLoadConfig(MatDataPath)
    ...
    configMP_GlobVectors(MeshPartList, F, M, Fixed)
