...
def updatePenaltyData(PenaltyData, IntfcElemList, IntfcElemList_Un):
    ...
    for p in range(N_IntegrationPoints_2D):
        ...
        for i in range(N_IntfcElem):
            ...
            Damage = np.min([1.0, np.max([NormalGap/dnc, SlideGap/dsc])])
            
            if NormalGap > 0: # Keep ">=" or ">" ?
                
                if Damage > DamageData[p,i]: #Loading (Increases Damage)
                    DamageData[p,i]=Damage
                    Ksm = getShearSecStiff(SlideGap)
                    Knn = getNormSecStiff(NormalGap)
                else: #Unloading/Loading
                    Ksm = getShearSecStiff(DamageData[p,i]*dsc) #irreversible
                    Knn = getNormSecStiff(DamageData[p,i]*dnc) #irreversible
...
def updatePenaltyData_Backup(PenaltyData, IntfcElemList, IntfcElemList_Un):
...
def plotLoadDispData(PenaltyData, PlotFileName, LoadDispData):
    TSL_Param = PenaltyData['TSL_Param']
    ft = TSL_Param['ft']*1e-6
    dn0 = TSL_Param['dn0']
    dnc = TSL_Param['dnc']
    
    fig = plt.figure()
    
    plt.plot([0, dn0, dnc], [0, ft, 0], '--k')
    
    """
    GapList = np.pi*np.arange(100)/100
    NormalTraction = ft*np.sin(GapList)
    plt.plot((wc/np.pi)*GapList, NormalTraction, '-k')
    """
    
    plt.plot(LoadDispData['Disp'], LoadDispData['Load'], '.-')
    ...
    for TimeStepCount in range(MaxTimeStepCount):
        ...
        if PlotFlag == 1:
            if PlotNDofs > 0:
                ...
                """
                """
                ShrGap = PenaltyData['GapVecData'][0, 0, 0] 
                PlotGapVector[TimeStepCount+1] = ShrGap
                Kss = PenaltyData['getShearSecStiff'](ShrGap)
                Tr = Kss*ShrGap
                PlotLoadVector[TimeStepCount+1] = Tr
                plotDispVecData(PlotFileName, PlotLoadVector, PlotGapVector)
                """
