...
def readInputFiles(MatPath):
    ...
    MeshFile_IntfcKeDataFile =    [MatPath + 'IntfcKe_' + str(i+1) + '.mat' for i in range(N_Splits)]
    ...
    for i in range(N_Splits):
        ...
        IntfcKe_Data_i = scipy.io.loadmat(MeshFile_IntfcKeDataFile[i])
        IntfcKe_i = [a for a in IntfcKe_Data_i['RefIntfcKe'][0]]
        IntfcKe += IntfcKe_i
    ...
    Type =      np.array(scipy.io.loadmat(MeshFile_Type)['Type'][0],dtype=int) - 1
    Ke =        scipy.io.loadmat(KeDataFile)['Ke'][0]
    
    return DofGlb, Sign, Type, Level, M, Ke, IntfcKe
...
def readLoadConfig(MatPath):
    ...
    return Fixed, F, RefPlotDofVec, qpoint, UInit, GlobData
...
def configMP_ElemVectors(MeshPartList, ElePart, DofGlb, Type, Level, Sign, IntfcKe): 
    ...
    for i in range(N_MeshPart):
        ...
        for RefElemId in RefElemIdList:
            ...
            MP_ElemList_IntfcKe.append(IntfcKe[RefElemId])
        ...
        MeshPart['ElemList_IntfcKe'] = MP_ElemList_IntfcKe
...
def configMP_GlobVectors(MeshPartList, F, M, Fixed, qpoint, UInit, RefPlotDofVec):
...
def configMP_TypeGroupList(MeshPartList):
    ...
    for i in range(N_MeshPart):
        ...
        MP_ElemList_IntfcKe = np.array(MeshPartList[i]['ElemList_IntfcKe'])
        ...
        for j in range(N_Type):
            ...
            MP_TypeGroup['ElemList_IntfcKe'] = MP_ElemList_IntfcKe[I]
...
def main(p, ScratchPath):
    ...
    #Reading Matlab Input Files
    MatDataPath = ScratchPath + 'ModelData/Mat/'
    DofGlb, Sign, Type, Level, M, Ke, IntfcKe = readInputFiles(MatDataPath)
    Fixed, F, RefPlotDofVec, qpoint, UInit, GlobData = readLoadConfig(MatDataPath)
    ElePart = getElePart(MatDataPath, p)

    
    #Converting Data
    MeshPartList = []
    configMP_ElemVectors(MeshPartList, ElePart, DofGlb, Type, Level, Sign, IntfcKe)
    configMP_GlobVectors(MeshPartList, F, M, Fixed, qpoint, UInit, RefPlotDofVec)
    configMP_TypeGroupList(MeshPartList)
    configMP_ElemStiffMat(MeshPartList, Ke)
    configMP_Neighbours(MeshPartList)
