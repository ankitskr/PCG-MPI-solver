...
def initGlobData():
    ...
    GlobData['PyDataPath_Part'] = PyDataPath_Part
...
def readModelData(GlobData):
    ...
    t1_             = time()
    N_MshPrt        = N_Workers
    PyDataPath      = GlobData['PyDataPath_Part']
...
def readGlobalSettings(GlobData):
    ...
    GlobSettingsFile    = '__pycache__/GlobSettings.zpkl'
    GlobSettings        = importz(GlobSettingsFile)
    TimeHistoryParam = GlobSettings['TimeHistoryParam']
    SolverParam = GlobSettings['SolverParam']
    
    GlobData['FintCalcMode']            = 'outbin' #'inbin', 'infor', 'outbin'
    GlobData['ExportVars']              = TimeHistoryParam['ExportVars']
    GlobData['PlotFlag']                = TimeHistoryParam['PlotFlag']
    GlobData['ExportFlag']              = TimeHistoryParam['ExportFlag']
    GlobData['ExportKeyFrm']            = TimeHistoryParam['ExportFrmRate']
    GlobData['ExportFrms']              = TimeHistoryParam['ExportFrms']
    GlobData['TimeStepDelta']           = TimeHistoryParam['TimeStepDelta']
    GlobData['RefMaxTimeStepCount']     = len(GlobData['TimeStepDelta'])
    GlobData['EnergyFlag']              = 0
    
    GlobData['MaxIter']                 = SolverParam['MaxIter']
    GlobData['Tol']                     = SolverParam['Tol']
...
def initExportData(RefMeshPart):
    ...
    if Rank==0:
        GlobData['TimeList_Flag'] = np.zeros(RefMaxTimeStepCount)
        GlobData['TimeList_RelRes'] = np.zeros(RefMaxTimeStepCount)
        GlobData['TimeList_Iter'] = np.zeros(RefMaxTimeStepCount)
    ...
    if PlotFlag == 1:
        ...
        if MP_PlotNDofs > 0:
            ...
            MP_PlotDispVector[:,0]          = RefMeshPart['Un'][MP_PlotLocalDofVec]
...
def updateBC(RefMeshPart):
    ...
    MP_Udi = RefMeshPart['Ud']*TimeStepDelta[TimeStepCount]
    MP_Fdi = calcMPFint(MP_Udi, RefMeshPart)
    MP_Fi = MP_RefLoadVector*TimeStepDelta[TimeStepCount]
    RefMeshPart['Fext'] = MP_Fi - MP_Fdi
    RefMeshPart['Udi'] = MP_Udi
...
def calcMatVecProd(RefMeshPart, ComputeReference='Strain', MP_Xn = []):
    ...
    for j in range(N_Type):        
        ...
        if ComputeReference == 'Strain':
            ...
            DiagK              = RefTypeGroup['ElemDiagStiffMat']
            ElemList_Ck        = RefTypeGroup['ElemList_Ck']
            
            ElemList_RefVec     =  ElemList_Ck*DiagK[np.newaxis].T
...
def PCG(RefMeshPart):
    ...
    MP_X0                               = RefMeshPart['Un']
    MP_Fext                             = RefMeshPart['Fext']    
    ...
    if Rank==0:
        GlobData['TimeList_Flag'][TimeStepCount] = Flag
        GlobData['TimeList_RelRes'][TimeStepCount] = RelRes
        GlobData['TimeList_Iter'][TimeStepCount] = Iter
    
    RefMeshPart['Un'] = MP_X_Unq + RefMeshPart['Udi']
...
def updateElemStrain(RefMeshPart):
    ...
    for j in range(N_Type):        
        ...
        ElemList_Un             = RefMeshPart['Un'][ElemList_LocDofVector]
        ElemList_Un[ElemList_SignVector] *= -1.0        
...
def exportTimeData(RefMeshPart):
    ...
    if Rank == 0:        
        ...
        TimeData['Flag']        = GlobData['TimeList_Flag']
        TimeData['Iter']        = GlobData['TimeList_Iter']
        TimeData['RelRes']      = GlobData['TimeList_RelRes']
        GlobData['TimeData']    = TimeData
...
if __name__ == "__main__":
    ...
    if Rank==0: print('>loading partitioned data..')
    ...
    if Rank==0: print(f">exporting results..")
    exportHistoryPlotData(RefMeshPart)
    exportTimeData(RefMeshPart)
    
    if Rank==0:    
        msg = f"""
>file read time:     {np.round(GlobData['TimeData']['Mean_FileReadTime'],1)} sec 
>calculation time:   {np.round(GlobData['TimeData']['Mean_CalcTime'],1)} sec
>communication time: {np.round(GlobData['TimeData']['Mean_CommWaitTime'],1)} sec
>---------------------------
>total runtime:      {np.round(GlobData['TimeData']['TotalTime'],1)} sec 
        """
        print('>success!')
        print(msg)
